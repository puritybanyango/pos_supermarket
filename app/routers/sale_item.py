...
def create_sale_item(sale_id: int, product_id: int, quantity: int, db: Session = Depends(get_db)):
    # 1. Prevent invalid database records as required by your prompt instructions
    ...
    # 2. Derive price calculations
    unit_price = product.price
    total_price = Decimal(str(unit_price)) * Decimal(str(quantity))

    # 3. Save record and deduct stock level
    new_item = SaleItem(
        sale_id=sale_id,
    ...
    product.stock_qty -= quantity  # Deduct stock units safely
...
def update_sale_item_quantity(id: int, new_quantity: int, db: Session = Depends(get_db)):
    ...
    # Calculate stock adjustment delta difference
    ...
    # Adjust stock and update mathematics metrics
...
def delete_sale_item(id: int, db: Session = Depends(get_db)):
    ...
    # Return allocated quantity inventory units back into stock before removing line
    product = db.query(Product).filter(Product.id == item.product_id).first()
    if product:
